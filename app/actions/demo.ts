...
const DEMO_EMAIL = "demo@financetracker.dev"
const DEMO_PASSWORD = "demo123456"

async function seedDemoData(userId: string) {
  const categories = await prisma.category.createManyAndReturn({
    data: [
      { name: "Salary", color: "#14b8a6", icon: "💼", userId },
      { name: "Food", color: "#f97316", icon: "🍔", budget: 500, userId },
      { name: "Transport", color: "#3b82f6", icon: "🚗", budget: 200, userId },
      { name: "Entertainment", color: "#8b5cf6", icon: "🎮", budget: 100, userId },
      { name: "Health", color: "#22c55e", icon: "💊", budget: 150, userId },
      { name: "Shopping", color: "#ec4899", icon: "🛍️", budget: 300, userId },
    ],
  })
...
export async function loginAsDemo() {
  ...
  if (!user) {
    ...
    user = await prisma.user.create({
      data: { name: "Demo User", email: DEMO_EMAIL, password: hashed },
    })
