...
function ChangeTag({
  ...
  if (pct === 0) return <span className="text-xs text-gray-400">= vs last month</span>
  const isUp = pct > 0
...
export default function BalanceCards({ balance, income, expenses, allTimeBalance, prevIncome, prevExpenses }: Props) {
  const savingsRate = income > 0 ? Math.round(((income - expenses) / income) * 100) : null
  const savingsColor = allTimeBalance > 0 ? "text-green-400" : allTimeBalance < 0 ? "text-red-400" : "text-white"
          ...
          <div className="flex items-center justify-between mt-1">
            ...
            {savingsRate !== null && (
              <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                savingsRate >= 20 ? "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400" :
                savingsRate >= 0 ? "bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400" :
