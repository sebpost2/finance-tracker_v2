...
export default function ProfileForm({ name, email }: { name?: string; email?: string }) {
  ...
  useEffect(() => {
    if (state?.success) showToast(t.profile.saved)
  }, [state?.success, showToast, t.profile.saved])

  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-6">
      <h2 className="text-base font-semibold text-gray-900 dark:text-white mb-1">{t.profile.section}</h2>
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-5">{t.profile.sectionHint}</p>
